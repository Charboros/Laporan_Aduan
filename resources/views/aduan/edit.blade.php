<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl leading-tight">
            Edit Aduan — <span class="text-blue-700">{{ $aduan->nomor_aduan }}</span>
        </h2>
    </x-slot>
            ...
            <form action="{{ route('aduan.update', $aduan->id) }}" method="POST"
                ...
                <div>
                    ...
                    <textarea name="isi_aduan" rows="4" required
                        class="w-full border-gray-300 rounded-md shadow-sm text-sm focus:border-blue-500 focus:ring focus:ring-blue-200">{{ old('isi_aduan', $aduan->isi_aduan) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    @if($aduan->screenshot_path)
                        <div class="mb-3 p-3 bg-gray-50 rounded border border-gray-200">
                            <p class="text-xs text-gray-500 mb-2">Screenshot saat ini:</p>
                            <img src="{{ asset('storage/' . $aduan->screenshot_path) }}"
                                alt="Screenshot" class="max-h-40 rounded border">
                ...
                <div class="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                    ...
                    <div id="isiResponDiv" class="{{ old('sudah_direspon', $aduan->sudah_direspon) ? '' : 'hidden' }} mt-3">
                        ...
                        <textarea name="isi_respon_awal" rows="3"
                            ...
                            class="w-full border-gray-300 rounded-md shadow-sm text-sm focus:border-blue-500 focus:ring focus:ring-blue-200">{{ old('isi_respon_awal', $aduan->isi_respon_awal) }}</textarea>
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-3 pt-2">
                    ...
                    <button type="submit"
                            class="px-5 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 text-sm font-medium">Update Aduan</button>
                </div>
            </form>
        </div>
    </div>
